...
export default class extends Controller {
  ...
  // Example: this.element.dispatchEvent(new CustomEvent("toast:show", { detail: { ... } }))
  show(event) {
    const { title, description, variant = "default", duration, action } = event.detail || {}
    this._addToast({ title, description, variant, duration: duration || this.durationValue, action })
  }

  success(event) {
    this._addToast({ ...event.detail, variant: "success", duration: event.detail?.duration || this.durationValue })
  }

  error(event) {
    this._addToast({ ...event.detail, variant: "error", duration: event.detail?.duration || this.durationValue })
  }

  warning(event) {
    this._addToast({ ...event.detail, variant: "warning", duration: event.detail?.duration || this.durationValue })
  }

  info(event) {
    this._addToast({ ...event.detail, variant: "info", duration: event.detail?.duration || this.durationValue })
  }
